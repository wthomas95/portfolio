...
window.onload = function() {
  ...
  canvas.height = 600;
  start();
  // canvas.onmouseover = function(hover){
  //   timer = 3000
  //   start();
  // };
};

//dropdown button for HERE app
function dropdown1(){
  document.getElementById("hereInfoContent").classList.toggle("show");
}
window.onclick = function(event) {
  if (!event.target.matches('.hereInfo')) {

    var dropdowns = document.getElementsByClassName("hereInfoContent");
    var i;
...
//dropdown button for JS game
function dropdown2(){
  document.getElementById("JSInfoContent").classList.toggle("show");
}
window.onclick = function(event) {
  if (!event.target.matches('.JSInfo')) {

    var dropdowns = document.getElementsByClassName("JSInfoContent");
    var i;
    for (i = 0; i < dropdowns.length; i++) {
      var openDropdown = dropdowns[i];
      if (openDropdown.classList.contains('show')) {
